...
\newcommand{\ArduinoJoystickVideoFootnote}{\footnote{Video - \url{https://www.youtube.com/watch?v=-ddtmqoGNmg}}}
...
The Arduino Uno R3 features an ATMega16U2\atmegaFootnote{} microcontroller that serves as a USB-to-serial bridge between the Arduino's central ATmega328\atmegaTFootnote{} microntroller \& the host computer. In its stock state, the ATMega16U2's firmware simply serves to allow communication between the ATMega328 (which has no native USB interface) \& a modern computer where USB connections are commonplace, appearing to the host computer's operating system as a generic USB-to-serial bridge.

The ATMega16U2 can have its firmware modified/replaced to perform other tasks. Such as pretending to be a USB joystick

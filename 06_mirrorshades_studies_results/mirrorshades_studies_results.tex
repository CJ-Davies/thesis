...
\subsection{Graphical Performancee of Mirrorshades Platform}

The overhead of capturing, processing and rendering the camera streams resulted in an overall lower and more variable framerate throughout the parallel reality scenario than the seated VR scenario. Figure \ref{1-frames-2-up.png} shows framerate against time for participant 1 performing both scenarios and this figure is representative of all participants throughout the stage 1 investigations. Note that the referesh rate of the DK1 is 60Hz and the Mirrorshades Unity application was run with vsync enabled (vsync limits framerate to the refresh rate of the display, in order to avoid screen tearing) so any values shown above 60 frames per second in figure \ref{1-frames-2-up.png} are due to the method used to estimate frames per second values. The periods in the seated VR scenario plot in figure \ref{1-frames-2-up.png} where the plot is roughly level around 60 frames per second represents when the Unity application had reached this vsync limit.

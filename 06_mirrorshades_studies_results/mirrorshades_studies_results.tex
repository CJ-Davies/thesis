...
Previously when VR has been employed in virtual heritage scenarios, it has predominantly been implemented via CAVE experiences~\cite{Roussou2002}. Visitors to a museum or visitor centre will be offered the opportunity to step into a CAVE, possibly donning shutter glasses or similar apparatus to enable a stereoscopic 3D effect, to experience a VR reconstruction of the locations, its contents \& actors. While some of these CAVE installations featured physical control interfaces such as joystick \& 3D mouse~\cite{cabral:x3dexperience} \& even haptic interfaces~\cite{Christou2006}, others have tracked user movement (whether just head, or full body  such as in figure \ref{VTTP_projection.png}).

One point in common to these CAVE experiences is that they are experienced with a disconnect to the RW site that their VR visuals represent. The CAVE itself immerses users in VR visuals \& does not permit them to see any of their RW surroundings. Furthermore, the physical size of the CAVE limits the amount in any particular direction that a user can move.

With the promise of high performance VR HMDs at a consumer price point on the horizon, thanks largely to the rejuvenation in the field effect by Oculus, their use in place of a CAVE is becoming more legitimate. The OVW group's experience with presenting both experts \& interested amateurs, young \& old, with virtual heritage content via Oculus HMDs (see section \ref{virtual-heritage-at-st-andrews}) has been very promising. Thus far these interactions have taken place in a physically static scenario; that is, the user remains in one place when standing or sitting with the HMD \& uses an Xbox controller, keyboard \& mouse or NUI gestures to control movement of their virtual presence.

In this first stage of the evaluation a comparison is made between this `traditional' style of interacting with VR content at a cultural heritage site, wherein the virtual is experienced in isolation from the real with both temporal \& spatial separation \& the `new' style afforded by the Mirrorshades platform in which the virtual can be experienced at the same time as the real by allowing the user to move around their RW environment \& transition into seeing the VR environment from the equivalent vantage point at any time.
...
Participants were first givn the opportunity to familiarise themselves with the Rift by spending a few minutes interacting with Oculus' `Tuscany' demo\footnote{\url{https://share.oculus.com/app/oculus-tuscany-demo}}. This demo is distributed by Oculus with the Rift Unity integration

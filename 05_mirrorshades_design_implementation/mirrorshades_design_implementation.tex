...
A further consideration is the discrepancy between the cameras' sensors \& the users' eyes, caused due to the fact that the cameras must be mounted to the front of the DK1 \& thus several inches in front of the user's eyes. This has the effect of making the user experience viewing the real world from several inches in front of where their eyes truly are (as if their eyes were `on stalks'), whilst viewing VR through the same setup does not have this effect. The distance between the sensors \& the user's eyes was reduced when iterating from the first mounting mechanism with metal stand-offs to the second mechanism with rubber washers, however with the interaction style of Mirrorshades where the user is predominantly focussed on observing objects \& architecture 15-20ft away from them the discrepancy is not particularly noticeable - it is when trying to manipulate objects much closer to the eyes that the discrepancy becomes prominent.

%With a HMD like the DK1, the screen's surface in these discussions is the virtual screen surface that the eye focusses upon, which is different to the physical screen surface. Although the physical screen surface of the DK1 sits only a few inches from the user's face, the virtual screen surface that they actually focus upon appears to be at infinity, thanks to the combination of optics \& rendering shader.

%but always focus on infinity
...
This needs to come before IndoorAtlas \& Unity section, as using the laptop is important for why the whole MySQL step in the middle was used.

Talk about Unity being inherently multiplatform, but there only being Rift support for Windows \& Mac OS at the time (no Android support \& Gear VR wasn't even announced). But mention that Gear VR would now be the ideal platform for this, as it would cut down on so much weight \& complexity \& allow the mobile client \& the IPS to be the same physical device rather than two separate ones - Gear VR even has that convenient input on the side so the controller wouldn't even be strictly necessary.

Picture from video of ODROID working (at least headtracking) with Rift, maybe link to video.
...
IndoorAtlas works on phone, assumes application using the position will be on same phone
	so modified example app to submit data via PHP/HTTP POST to a MySQL database for use by other things
	
Web pages (both Jack Cole \& Sallies) that used this information

Built Unity application for Android that showed topdown view of Jack Cole using this information
\url{https://www.youtube.com/watch?v=i3lEnXZMjms}
Have screenshot from this video

Haversine implementation in Unity

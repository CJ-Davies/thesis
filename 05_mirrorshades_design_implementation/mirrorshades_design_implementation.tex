...
For outdoor applications, GPS represents a suitable solution for the vast majority of position tracking requirements. Global coverage \& the ability to scale accuracy as required, from many metres with a basic GPS receiver such as might be integrated into a smartphone, to a few metres with SBAS augmentations \& further to as little as 10cm with the deployment of Differential GPS (DGPS) beacons, has led to GPS occupying the role of the `go to' solution where position tracking is required for an outdoor application. For indoor applications however, there is no single technology or solution that provides similar coverage or suitability as GPS does outside: a large number of different technologies have been employed to produce Indoor Positioning Systems (IPS). At the highest level of abstraction, all IPS can be divided into three categories based upon the technology that they employ;

\begin{itemize}
	\item inertial navigation (using accelerometers \& gyroscopes)
	\item mechanical waves (audible \& ultra-sound)
	\item electromagnetic waves (visible \& infrared light, microwaves, radio waves)~\cite{Mautz2012}
\end{itemize}

Because of this diversity in technology, thus also diversity in performance \& suitability to different situations, it is necessary to carefully consider the requirements of the platform \& then choose the best suited of the many different approaches. Requirements to consider include accuracy, coverage, update frequency, latency, cost, infrastructure, privacy, robustness \& intrusiveness. Unsurprisingly, selection of a particular IPS usually leads to balancing these requirements in a trade-off, as each of the challenges of indoor positioning effects each technology more or less than others~\cite{Mautz2009}.

Considering these requirements for the Mirrorshades platform;

The accuracy required for the Mirrorshades platform needs to be substantially better than that observed during the VTW experiments. As a pedestrian application wherein the user walks through doorways (whether real or virtual) \& observes multiple rooms within a building, it is necessary to achieve a level of accuracy that allows, for example, discerning between adjacent rooms \& discerning between a doorway \& its surrounding walls.

Coverage depends largely upon the size of the cultural heritage site that Mirrorshades is deployed to. However it is prudent to make use of an IPS that can scale quite arbitrarily from small scenarios (perhaps of a small village church) to substantially larger scenarios (such as a cathedral similar to that at St Andrews).

A high update frequency is not especially important to Mirrorshades. The envisaged style of interaction is one wherein users walk relatively slowly through the environments, as they wish to observer \& take in their surroundings. Updates in the range of several hz will likely be sufficient, especially if users are attending more to their real environment than the equivalent virtual environment when actively moving around (which is to be encouraged, as one cannot walk through a RW obstacle as one can a VR one!).

Similarly, low latency is not critical. Even if the IPS takes a few seconds to `catch up' with the user, as the user is committed to a deliberate study \& comparison of their real \& virtual surroundings they are not going to be foiled in their task if they find they have to wait momentarily when switching from real to virtual views.

Cost

%https://www.youtube.com/watch?v=LtMfrkRqlRs
email from xsens confirming price
``A complete standard Xsens MVN system is available at around \euro{}50000''
...
email from Sonitor
To cover ground floor and have room level accuracy in each room + tracking
in the corridors, the cost would be ca. \$25K (includes software)
Adding the first floor (room accuracy in all rooms + corridor tracking)
would add another \$15K (assumes software already included in ground floor)
...
from vtw we know that accuracy has to be greater than that achievable from a standalone GPS receiver, being on the wrong side of a virtual wall is not sufficient
for a pedestrian application

infrastructure - many cultural heritage sites will not allow the installation of any infrastructure into the site/environment, or may only allow strictly temporary infrastructure to be deployed, which means that approaches that require extensive infrastructure to be deployed or for which the deployment \& calibration phase is long \& thus not suitable for temporary deployments unusable.
